function buildTablePR(json, jsonOnkeyp = true) {
    document.getElementById("containerTablePrivilegesRoles").innerHTML = "";
    var ban = false;
    var cont = 1;
    var html = `           
                  <div>
                      ...
                      <table class="table table-striped jambo_table bulk_action" id="tbl-roles">
                        <thead>
                          <tr class="headings">
                            ...
                            <th class="column-title" style="display: table-cell;">Privilegios</th>
        ...
        for (var i = 0; i < json.length; i++) {
            ...
            html +=
                `<tr class="even pointer">
                                    ...
                                    <td class=" ">${cont++}</td>`
            html += ` <td class=" ">${json[i].rol}</td>`
            html += ` <td class=" ">${json[i].privilegios}</td>`
    ...
    if (ban) {
        document.getElementById("containerTablePrivilegesRoles").innerHTML = html;
        initChecksTable();
        addEventCheckAll();
    } else if (!jsonOnkeyp) {
        document.getElementById("containerTablePrivilegesRoles").innerHTML = "<h4 class='text-center'>No se encontro ninguna coincidencia</h4>"
    } else {
        document.getElementById("containerTablePrivilegesRoles").innerHTML = "<h4 class='text-center'>Aún no hay registros agregados</h4>"
    }
